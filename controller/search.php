...
(user_subjects.subject_name like '%" . $key . "%' or users.locality like '%" . $key . "%' or 
		...
		$search_users = mysqli_query ( $connection, "select users.id,users.name,users.locality,users.city,users.class,user_subjects.subject_name
                                                      from users inner join user_subjects on users.id=user_subjects.user_id
...
(user_subjects.subject_name like '%" . $key . "%' or users.locality like '%" . $key . "%' or
			...
			$search_users_perpage = mysqli_query ( $connection, "select users.id, users.name,users.locality,users.city,users.class,user_subjects.subject_name
                                                      from users inner join user_subjects on users.id=user_subjects.user_id
                                                     where users.usertype='student' and
(user_subjects.subject_name like '%" . $key . "%' or users.locality like '%" . $key . "%'
